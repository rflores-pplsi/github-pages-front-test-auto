...
export class OktaPage extends LoginPage {
  ...
  loginThroughOkta = async (): Promise<void> => {
    ...
    const emailOrUsername = UsersUtils.oktaUser.email;
    const password = UsersUtils.oktaUser.password;
  ...
  navigateToPlanalyzerCsrCheckoutOktaLogin = async (): Promise<void> => {
    ...
    // Navigate to Okta
    await this.goTo(
      UrlsUtils.legalshieldInternalUrls.planalyzer_csr_checkout.url
    );
    // Wait for page to finish loading
    await this.page.waitForLoadState("networkidle");
  };
