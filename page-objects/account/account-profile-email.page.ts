...
let btnEmailAddressAdd = 'button.lsux-button.lsux-button--primary.mt-4 img';
let txtBoxEditEmailAddress = '//div[@class="lsux-container lsux-container--white  lsux-container--flexbox   lsux-container--flex-justify-space-between   mobile-profile-grid-container"]';
let btnConfirmEmailAddress = '//*[@class="inputs--wrapper inputs--wrapper-list"]/div/div[3]/button[1]';
let newAddedEmailValue ;
let txtBoxEditEmail = '[name="email"]';
let editedEmailValue;
...
// Page Instances
  ...
  addEmailAddressFun = async (email:string): Promise<void> =>{
    ...
    let emailAddressAdd = await this.page.$$(txtBoxEditEmailAddress);
    console.log(emailAddressAdd.length);
        for(const emailEle of emailAddressAdd){
    ...
    let confirmBtn = await this.page.$$(btnConfirmEmailAddress);
    ...
    await this.page.waitForSelector('//div[@class="inputs--wrapper inputs--wrapper-list"]/div['+(emailAddressAdd.length +1)+']/div/div/div/input');
  ...
  editEmailAddressFun = async (email:string, newEmail:string): Promise<void> =>{
    console.log(" - accountProfilePage.editPhoneNumberBtn")
        // Create an array that contains all Email text boxes 
        await this.page.waitForSelector(txtBoxEditEmailAddress);
        let emailAddressEdit = await this.page.$$(txtBoxEditEmailAddress);
        console.log(emailAddressEdit.length);
        for(let i=1;i<emailAddressEdit.length+1;i++){
    let oEmail = await (this.page.locator('//*[@class="inputs--wrapper inputs--wrapper-list"]/div['+i+']/div[2]/div/div[@class="lsux-input-container"]/input')).getAttribute('value');
    console.log(oEmail);
        }
    ...
    if(oEmail===email){
      ...
      await this.fillTextBox('//*[@class="inputs--wrapper inputs--wrapper-list"]/div['+i+']/div[2]/div/div[@class="lsux-input-container"]/input',newEmail);
      await this.page.click('//*[@class="inputs--wrapper inputs--wrapper-list"]/div['+i+']/div[3]/button[1]',{force: true});
    }
