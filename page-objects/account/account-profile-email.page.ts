...
const btnConfirmEmailAddress =
  '//*[@class="inputs--wrapper inputs--wrapper-list"]/div/div[3]/button[1]';
let newAddedEmailValue;
const txtBoxEditEmail = '[name="email"]';
...
export class AccountProfileEmailPage extends AccountProfilePage {
  ...
  addEmailAddressFun = async (email: string): Promise<void> => {
    ...
    await this.page.waitForSelector(
      '//div[@class="inputs--wrapper inputs--wrapper-list"]/div[' +
        (emailAddressAdd.length + 1) +
        "]/div/div/div/input"
    );
  ...
  editEmailAddressFun = async (
    ...
    for (let i = 1; i < emailAddressEdit.length + 1; i++) {
      const oEmail = await this.page
        .locator(
          '//*[@class="inputs--wrapper inputs--wrapper-list"]/div[' +
            i +
            ']/div[2]/div/div[@class="lsux-input-container"]/input'
        )
        .getAttribute("value");
      console.log(oEmail);
    }
  ...
  assertProfileUpdatedEmailTxtBox = async (email: string): Promise<void> => {
    ...
    const updatedEmail = await this.page?.locator(
      '//input[@value="' + email + '"]'
    );
    expect(await updatedEmail.getAttribute("value")).toBe(email);
    console.log(
      "The updated Email: " +
        (await updatedEmail.getAttribute("value")) +
        " is equal to the email: " +
        email
    );
    console.log(email);
  ...
  assertProfileAddEmailTxtBox = async (email: string): Promise<void> => {
    console.log(" - profilePhoneNumberPage.assertProfilePhoneNumberTxtBox");
    // Confirm the Email Address  is updated
    const addedEmail = await this.page?.locator(
      '//input[@value="' + email + '"]'
    );
    expect(await addedEmail.getAttribute("value")).toBe(email);
    console.log(
      "The added Email: " +
        (await addedEmail.getAttribute("value")) +
        " is equal to the email: " +
        email
    );
    console.log(email);
