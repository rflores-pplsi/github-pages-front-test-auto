...
const TAB_COMMERCIAL_DRIVER_PAGE = '#root > div > nav > div > div > ul > li:nth-child(6) > a';
...
const TXT_LAW_FIRM_NAME = '.lsux-col.col.four > div > p > h5';
...
const BTN_VIEW_DETAILS = '#root div:nth-child(3) > div > div:nth-child(1) > div > div > div.groupTokenCardLeft > a > img';
...
export class ShieldBenefitsCommercialDriverPage extends OktaPage {
   ...
   * @memberof ShieldBenefitsCommercialDriverPage
   */
  navigateToGroupEnrollmentCommercialDriverPage = async (): Promise<void> => {
  ...
  clickBtnCommercialDriverPage = async (): Promise<void> => {
    ...
    await this.clickOnElement(TAB_COMMERCIAL_DRIVER_PAGE);
  };
  ...
  assertShieldBenefitsCommercialDriverPage = async (): Promise<void> => {
    ...
    await this.assertElementIsVisible(TAB_COMMERCIAL_DRIVER_PAGE);
  };
  ...
  assertButtonViewDetailsIsDisplayed = async (): Promise<void> => {
    ...
    await this.page.waitForSelector(BTN_VIEW_DETAILS);
  };
