...
const BTN_OBTENIR_UNP_LAN = '#block-is-business-builder-plan >> text=OBTENIR UN PLAN';
...
const LNK_CHANGE_SHOPPING_REGION = '//a[contains(text(),"Change Shopping Region")]';
...
export class FrenchWalsCaPage extends OktaPage {
  ...
  changeStateinformation = async (state: string): Promise<void> => {
    ...
    await this.page.waitForSelector(LNK_CHANGE_SHOPPING_REGION);
    await this.page.locator(LNK_CHANGE_SHOPPING_REGION).click({ force: true });
  ...
  clickConsigueUnPlanBtn = async (): Promise<void> => {
    ...
    await this.page.locator(BTN_OBTENIR_UNP_LAN).click();
  };
