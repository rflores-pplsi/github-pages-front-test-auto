...
const BTN_PRÓXIMO = '#builder_modal_checkout_btn_continue';
...
const LNK_CHANGE = '//div/a[contains(text(),"Change Shopping Region")]';
...
const RDO_BUTTON_USERNAME = '.mat-radio-container';
...
export class SpanishWalsUSPage extends OktaPage {
  ...
  createAUser = async (pass: string, confirmpass: string): Promise<void> => {
    ...
    // Select a username
    await this.page.waitForSelector(RDO_BUTTON_USERNAME);
    const radioUsernames = await this.page.$$(RDO_BUTTON_USERNAME);
  ...
  clickPRÓXIMOBtn = async (): Promise<void> => {
    ...
    // Click on Next button
    await this.clickOnElement(BTN_PRÓXIMO);
  };
