...
const BTN_GET_A_PLAN = '#block-is-business-builder-plan >> text=GET A PLAN';
...
const SELECT_PHONE_TYPE = '//span[contains(text(),"Phone Type")]';
...
export class EnglishWalsUSPage extends OktaPage {
  ...
  createAUser = async (pass: string, confirmpass: string): Promise<void> => {
    ...
    // Confirm Password
    await this.page.waitForSelector(TXT_CONFIRM_PASSWORD);
  ...
  clickGetStartedBtn = async (): Promise<void> => {
    ...
    await this.page.locator(BTN_GET_A_PLAN).click();
  };
