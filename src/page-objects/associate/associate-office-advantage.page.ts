...
assertAdvantagePlusContainer = async (): Promise<void> => {
  ...
  const description3 = 'LSEngage: Customizable top-level statistical reporting to build and manage your business including a tool to visualize your organization';
...
assertSupportContentIsDisplayed = async (): Promise<void> => {
  ...
  await this.page.waitForSelector(plansHeader);
  const strSupportContent = 'Feature upgrades will be available within 24 hours. Feature downgrades and new charges will take effect at your next bill date. You can change your LegalShield Advantage plan every 30 days.';
