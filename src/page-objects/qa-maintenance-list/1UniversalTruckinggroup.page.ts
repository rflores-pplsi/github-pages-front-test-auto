...
const TXT_PLAN_JURIDIQUE = '//div[@class="clearfix"]/h4/ng-bind-html[contains(text(),"Plan juridique")]';
...
export class UniversalTruckingPage extends OktaPage {
  ...
  clickTabSigningUp = async (): Promise<void> => {
    ...
    await this.page.click(TAB_SIGNUP);
  };
  ...
  assertSelectedPlanAndParMoiTxt = async (): Promise<void> => {
    ...
    await this.page.waitForSelector(TXT_PLAN_JURIDIQUE);
    const plan = await this.page.$$(TXT_PLAN_JURIDIQUE);
    await plan[0].innerHTML();
    console.log(await plan[0].innerHTML());
    await this.assertElementHasText(TXT_PLAN_JURIDIQUE, 'Plan juridique');
