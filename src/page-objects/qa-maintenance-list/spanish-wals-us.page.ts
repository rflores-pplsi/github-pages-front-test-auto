...
const BTN_CONSIGUE_UN_PLAN = '#block-is-business-builder-plan >> text=CONSIGUE UN PLAN';
...
const BTN_PROXIMO = '#builder_modal_checkout_btn_continue';
...
const TTL_CONTACT_INFO = '//h2[contains(text(),"Información del contacto")]';
...
const SELECT_PHONE_TYPE = '//span[contains(text(),"Tipo de telefono")]';
const TXT_DATE_OF_BIRTH = '#date-birth';
const BTN_DEPENDENCY_FIRST_NAME = '#first-name-dependant-form';
const TXT_DEPENDENCY_LAST_NAME = '#last-name-dependant-form';
const TXT_DEPENDENT_B_DAY = '#date-birth-dependant-form';
const TXT_DEPENDENT_EMAIL = '#dependant-email-start-form';
const SELECT_FAMILY_MEMBER_TYPE = '//span[contains(text(),"Tipo de miembro de la familia")]';
...
const BTN_COMMISSION_OPTION_CONTINUE = '//button[contains(text()," Continuar ")]';
...
export class SpanishWalsUSPage extends OktaPage {
  ...
  changeStateinformation = async (state: string): Promise<void> => {
    ...
    await this.page.waitForSelector(LNK_CHANGE);
  ...
  filloutSecurityAndFamilyCoverageInfo = async (
    ...
    await this.typeTextBox(BTN_DEPENDENCY_FIRST_NAME, depFirst);
    // Fill Dependent Last Name
    await this.typeTextBox(TXT_DEPENDENCY_LAST_NAME, depLast);
  ...
  clickConsigueUnPlanBtn = async (): Promise<void> => {
    ...
    await this.page.locator(BTN_CONSIGUE_UN_PLAN).click();
  };
  ...
  clickPRÓXIMOBtn = async (): Promise<void> => {
    ...
    // Click on Next button
    await this.clickOnElement(BTN_PROXIMO);
  };
  ...
  assertContactInformationTxt = async (): Promise<void> => {
    ...
    await this.assertElementContainsText(TTL_CONTACT_INFO, 'Información del contacto');
  ...
  assertWelcomelabel = async (): Promise<void> => {
    ...
    await this.assertElementContainsText(LBL_WELCOME, '¡Bienvenido a la familia LegalShield!');
