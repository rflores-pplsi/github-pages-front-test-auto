...
const BTN_TERMS_OF_SERVICE = '#cc_form >> text=Terms of Service';
...
export class CheckoutPaymentsPage extends CheckoutPersonalInfoPage {
  ...
  assertTermsOfServiceNewTab = async (): Promise<Page> => {
    const [newPage] = await Promise.all([this.context.waitForEvent('page'), await this.page.click(BTN_TERMS_OF_SERVICE)]);
