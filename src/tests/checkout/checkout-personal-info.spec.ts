...
test.only('Verify Personal Information Section Header Displays', async ({ page }) => {
  ...
  await checkoutPersonalInfoPage.navigateToPaymentsPageForF30IdsCa('California');
...
test('Verify Error for all fields on Personal Info Page Displays', async ({ page }) => {
  ...
  await checkoutPersonalInfoPage.navigateToPersonalInfoPageFromPlanalyzer('D2C', 'LegalShield', 'California', 'en-US', '', '', [
    'commercial-drivers',
    'small-business',
  ]);
