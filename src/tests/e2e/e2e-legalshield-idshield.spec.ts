...
import legalshieldIdShieldMonthlyData from './data/legalshield-idshield/e2e-legalshield-idshield-monthly.json';
import legalshieldIdShieldAnnualData from './data/legalshield-idshield/e2e-legalshield-idshield-annual.json';
import legalshieldCanadaMonthlyData from './data/legalshield/e2e-legalshield-canada-monthly.json';
...
for (const tc of legalshieldIdShieldMonthlyData.filter((tc) => tc.disabled == false)) {
  for (const region of tc.regions) {
    test(`${tc.testCaseName} - ${region}`, async ({ page }) => {
      ...
      await test.step(`Assert Short Codes: ${JSON.stringify(tc.productDetails)}`, async () => {
...
for (const tc of legalshieldCanadaMonthlyData.filter((tc) => tc.disabled == false)) {
  for (const region of tc.regions) {
    test.only(`${tc.testCaseName} - ${region}`, async ({ page }) => {
      ...
      // Select Plans and get to Personal Info Page
      await test.step(`Navigate to Test Harness - LegalShield}`, async () => {
        await checkoutConfirmationPage.goTo(UrlsUtils.testHarnessUrls.legalShieldCanada.url);
      });
      await test.step(`Select Region: ${region}`, async () => {
        await checkoutConfirmationPage.selectRegionFromDropdown(region);
      });
      ...
      // Complete Necessary Forms and Continue to Payment Page
      await test.step(`Change Address to a valid one for Region: ${region}`, async () => {
        await checkoutConfirmationPage.changeAddressCanada(region);
      });
      await test.step(`Complete Business Form if Necessary`, async () => {
        await checkoutConfirmationPage.completeBusinessInfoForm();
      });
