/* eslint-disable sort-keys-fix/sort-keys-fix */
/* eslint-disable sort-keys */
import UrlsUtils from '../../../../utils/urls.utils';

export const legalshieldServiceData = [
  {
    disabled: false,
    pageName: 'Home',
    url: `${UrlsUtils.legalshieldService.baseUrl}/`,
    expectAddToCartButton: false,
  },
  {
    disabled: false,
    pageName: 'Personal Plan Plan Details (redirects to coverage and pricing) ',
    url: `${UrlsUtils.legalshieldService.baseUrl}/personal-plan/plan-details/`,
    expectAddToCartButton: false,
  },
  {
    disabled: false,
    pageName: 'Business Plan Coverage and Pricing',
    url: `${UrlsUtils.legalshieldService.baseUrl}/business-plan/coverage-pricing/`,
    expectAddToCartButton: true,
  },
  {
    disabled: false,
    pageName: 'Business Plan Plan Summary',
    url: `${UrlsUtils.legalshieldService.baseUrl}/business-plan/plan-summary/`,
    expectAddToCartButton: true,
  },

  {
    disabled: false,
    pageName: 'Legal Plans Overview',
    url: `${UrlsUtils.legalshieldService.baseUrl}/legal-plans-overview/`,
    expectAddToCartButton: true,
  },

  {
    disabled: false,
    pageName: 'Choose a Business Plan',
    url: `${UrlsUtils.legalshieldService.baseUrl}/business-plan/choose-a-plan/`,
    expectAddToCartButton: false,
  },

  {
    disabled: false,
    pageName: 'Business Plan FAQ',
    url: `${UrlsUtils.legalshieldService.baseUrl}/business-plan/faq/`,
    expectAddToCartButton: false,
  },
  {
    disabled: false,
    pageName: 'Business Licenses Federal and State Requirements',
    url: `${UrlsUtils.legalshieldService.baseUrl}/business-licenses/federal-and-state-requirements/`,
    expectAddToCartButton: false,
  },

  {
    disabled: false,
    pageName: 'Business Licenses DBA',
    url: `${UrlsUtils.legalshieldService.baseUrl}/business-licenses/dba/`,
    expectAddToCartButton: false,
  },

  {
    disabled: false,
    pageName: 'Business Licenses Business Permits',
    url: `${UrlsUtils.legalshieldService.baseUrl}/business-licenses/business-permits/`,
    expectAddToCartButton: false,
  },

  {
    disabled: false,
    pageName: 'FAQ',
    url: `${UrlsUtils.legalshieldService.baseUrl}/faq/`,
    expectAddToCartButton: false,
  },

  {
    disabled: false,
    pageName: 'Traffic Accident',
    url: `${UrlsUtils.legalshieldService.baseUrl}/traffic-accident/`,
    expectAddToCartButton: false,
  },

  {
    disabled: false,
    pageName: 'Renters',
    url: `${UrlsUtils.legalshieldService.baseUrl}/renters/`,
    expectAddToCartButton: false,
  },

  {
    disabled: false,
    pageName: 'Employment',
    url: `${UrlsUtils.legalshieldService.baseUrl}/employment/`,
    expectAddToCartButton: false,
  },

  {
    disabled: false,
    pageName: 'Employment Hiring and Firing',
    url: `${UrlsUtils.legalshieldService.baseUrl}/employment/hiring-and-firing/`,
    expectAddToCartButton: false,
  },

  {
    disabled: false,
    pageName: 'Employment Workers Comp',
    url: `${UrlsUtils.legalshieldService.baseUrl}/employment/workers-comp/`,
    expectAddToCartButton: false,
  },

  {
    disabled: false,
    pageName: 'Employment Ada',
    url: `${UrlsUtils.legalshieldService.baseUrl}/employment/ada/`,
    expectAddToCartButton: false,
  },
  {
    disabled: false,
    pageName: 'Family Law',
    url: `${UrlsUtils.legalshieldService.baseUrl}/family-law/`,
    expectAddToCartButton: false,
  },
  {
    disabled: false,
    pageName: 'How to Cancel Your Plan',
    url: `${UrlsUtils.legalshieldService.baseUrl}/legalshield-how-cancel-your-plan/`,
    expectAddToCartButton: false,
  },
  {
    disabled: false,
    pageName: 'Real Estate',
    url: `${UrlsUtils.legalshieldService.baseUrl}/real-estate/`,
    expectAddToCartButton: false,
  },
  {
    disabled: false,
    pageName: 'Landlords',
    url: `${UrlsUtils.legalshieldService.baseUrl}/landlords/`,
    expectAddToCartButton: true,
  },
  {
    disabled: false,
    pageName: 'Landlords Negotiating Leases',
    url: `${UrlsUtils.legalshieldService.baseUrl}/landlords/negotiating-leases/`,
    expectAddToCartButton: true,
  },
  {
    disabled: false,
    pageName: 'Landlords Tenant Without a Lease',
    url: `${UrlsUtils.legalshieldService.baseUrl}/landlords/tenant-without-a-lease/`,
    expectAddToCartButton: true,
  },
  {
    disabled: false,
    pageName: 'Landlords How to Evict a Tenant',
    url: `${UrlsUtils.legalshieldService.baseUrl}/landlords/how-to-evict-a-tenant/`,
    expectAddToCartButton: true,
  },
  {
    disabled: false,
    pageName: 'Landlords Eviction Defenses',
    url: `${UrlsUtils.legalshieldService.baseUrl}/landlords/eviction-defenses/`,
    expectAddToCartButton: true,
  },
  {
    disabled: false,
    pageName: 'Landlords Problem Tenant',
    url: `${UrlsUtils.legalshieldService.baseUrl}/landlords/problem-tenant/`,
    expectAddToCartButton: true,
  },
  {
    disabled: false,
    pageName: 'Landlords Security Deposit',
    url: `${UrlsUtils.legalshieldService.baseUrl}/landlords/security-deposit/`,
    expectAddToCartButton: true,
  },
  {
    disabled: false,
    pageName: 'Personal Plan Coverage and Pricing',
    url: `${UrlsUtils.legalshieldService.baseUrl}/personal-plan/coverage-and-pricing/`,
    expectAddToCartButton: true,
  },
  {
    disabled: false,
    pageName: 'Legal Protection Home Business',
    url: `${UrlsUtils.legalshieldService.baseUrl}/legal-protection-home-business/`,
    expectAddToCartButton: true,
  },
  {
    disabled: false,
    pageName: 'Extended Trial and Lawsuit Defense Protection',
    url: `${UrlsUtils.legalshieldService.baseUrl}/extended-trial-and-lawsuit-defense-protection/`,
    expectAddToCartButton: true,
  },
  {
    disabled: false,
    pageName: 'Gun Supplement',
    url: `${UrlsUtils.legalshieldService.baseUrl}/gun-supplement/`,
    expectAddToCartButton: true,
  },
  {
    disabled: false,
    pageName: 'Legal Protection Truck Drivers and Other Commercial Drivers',
    url: `${UrlsUtils.legalshieldService.baseUrl}/legal-protection-truck-drivers-and-other-commercial-drivers/`,
    expectAddToCartButton: true,
  },
  {
    disabled: false,
    pageName: 'Collection',
    url: `${UrlsUtils.legalshieldService.baseUrl}/collection/`,
    expectAddToCartButton: true,
  },
  {
    disabled: false,
    pageName: 'DM June Email 2 - LegalShield',
    url: `${UrlsUtils.legalshieldService.baseUrl}/dm-june-email2/`,
    expectAddToCartButton: true,
  },
];
