...
for (const regionUnderTest of regionsUnderTest) {
  test(`Legalshield (Legal Plan, en-CA, ${regionUnderTest}) -> Checkout -> Accounts @smoke `, async ({
    page,
    legalshieldService,
    ...
    if (process.env.USE_PROD == 'true') {
      ...
      await test.step(`Validate Order Summary on Payment Info Page`, async () => {
        expect(await commonCheckoutService.paymentPage.orderSummaryComponent.locTotalContainer.innerText()).toContain('$32.95');
      });
