...
for (const regionUnderTest of regionsUnderTest) {
  test(`Somoslegalshield (Legal Plan, es-US, ${regionUnderTest}) -> Checkout -> Accounts @smoke `, async ({
    page,
    walsService,
    newCheckoutService,
  }) => {
    ...
    await test.step('Navigate to somoslegalshield.com', async () => {
      await walsService.walsAffiliatedPage.navigateToAffiliatedWalsPage('lstestauto', 'somoslegalshield', 'com');
    });
