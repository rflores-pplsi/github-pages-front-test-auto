...
for (const testCase of legalshieldAssociateCanadaData.filter((testCase) => testCase.disabled == false)) {
  for (const regionUnderTest of testCase.regions) {
    test(`Legalshield Associates - Canada ${testCase.testCaseName}, ${regionUnderTest}) @legalshieldassociates-consumerflow @e2e ${testCase.tag}`, async ({
      page,
      legalshieldService,
      purchaseService: purchaseService,
      legalshieldAssociateService,
    }) => {
      console.log(`Test Case: Legalshield Associates - Canada(${testCase.testCaseName}, ${regionUnderTest})`);
      test.slow();
      const regionInfo = RegionsUtils.getProvinceInfo(regionUnderTest);
      await test.step(`Navigate to legalshieldassociate home page`, async () => {
        await legalshieldAssociateService.homePage.navigateToCanadianHomePage();
      });
      await test.step('Accept All Cookies', async () => {
        await legalshieldService.clickAcceptAllButton();
      });
      ...
      await test.step(`Fill Business Info Form`, async () => {
        if (testCase.planDetails?.some(plan => plan.marketingName === 'Small Business')) {
          await purchaseService.purchasePage.fillBusinessInfoForm('Test Business', '10101990', '111111111');
          await purchaseService.purchasePage.clickContinueButton();
        }
      });
      await test.step(`Fill Personal Info Form and Submit`, async () => {
        await purchaseService.purchasePage.fillPersonalInfoForm(
            basicUser.email,
            'Test',
            'PplsiTest',
            '5555555555',
      ...
      await test.step(`Click continue again to skip Additional Information Form`, async () => {
        await purchaseService.purchasePage.skipAdditionalInformationForm(testCase.planDetails);
      });
      await test.step(`Fill out Bank Draft Form and Continue`, async () => {
        await purchaseService.purchasePage.clickCreditCardBankDraftToggle();
        //TODO: revisit this wait and find an explicit one if possible
        await page.waitForTimeout(500);
        await purchaseService.purchasePage.fillPaymentInfoFormWithCanadianBankDraft('0000011', '11242', '260', 'Tester');
        await purchaseService.purchasePage.clickPaymentContinueButton();
      });
      await test.step(`Click Submit Button`, async () => {
