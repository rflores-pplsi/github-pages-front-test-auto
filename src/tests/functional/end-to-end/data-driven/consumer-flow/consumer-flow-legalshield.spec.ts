...
for (const testCase of legalshieldData.filter((testCase) => testCase.disabled == false)) {
  for (const regionUnderTest of testCase.regions) {
    test.fixme(
      `Legalshield (${testCase.testCaseName}, ${regionUnderTest}) @legalshield-consumerflow ${testCase.tags}`,
      async ({ page, legalshieldService, commonCheckoutService, commonLoginService }) => {
