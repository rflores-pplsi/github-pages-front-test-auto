...
import { CommonCheckoutService } from '../../../../node_modules/@legalshield/frontend-automation-commons';
...
test('D2E IDShield US using Testing Harness', async () => {
  ...
  await test.step('Navigate to Testing Harness', async () => {
    await testHarnessD2cPage.navigateToTestingHarnessPage('d2cIDShieldUS');
  });
  ...
  await test.step('Test from IDShield', async () => {
    await testHarnessD2cPage.clickOnALineOfBusiness(DataUtils.data.testingHarness.lineOfBusiness.IDShield, 'IDShieldUS');
  });
  ...
  await test.step('Add Plan and some Supplements', async () => {
    await testHarnessD2cPage.addPlanAndSomeSupplements('d2cIDShieldUS', [DataUtils.data.testingHarness.plans.us.IDShieldIndividual]);
  });
