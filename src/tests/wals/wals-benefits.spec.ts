...
test('I can review information on the hero banners.', async () => {
  ...
  await test.step('Verify that the banner header says Business Builder and the price says $49.90/month + $99.00 one time fee', async () => {
    ...
    await walsBenefitsPage.assertBannerPlanPrice('$49.90/month + $99.00 one time fee');
  });
  ...
  await test.step('Verify that the banner header says IDShield for Business and the price says $79.95/month', async () => {
    await walsBenefitsPage.assertBannerHeader('IDShield for Business');
    await walsBenefitsPage.assertBannerPlanPrice('$79.95/month');
  });
  ...
  await test.step('Verify that the banner header says Commercial Associate Startup and the price says $ 99.00 as a one time fee', async () => {
    await walsBenefitsPage.assertBannerHeader('Commercial Associate Startup');
    await walsBenefitsPage.assertBannerPlanPrice('$ 99.00 as a one time fee');
  });
  await test.step('Verify that the banner header says Legal & Identity and the price says $49.90/month', async () => {
    ...
    await walsBenefitsPage.assertBannerPlanPrice('$49.90/month >> nth = 1');
  });
});
