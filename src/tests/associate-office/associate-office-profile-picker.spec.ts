...
test('The Profile Picker Page is loading and the Left Nav Menu is not appearing', async ({ page }) => {
  await profilePickerPage.assertProfilePickerPage();
  await profilePickerPage.assertNoLeftNavMenuOnPage();
});

test('Verify that the Profile Picker has multiple accounts', async ({ page }) => {
...
test('One of the Profile Picker accounts is redirecting correctly', async ({ page }) => {
  await profilePickerPage.assertProfilePickerPage();
  await profilePickerPage.clickOnAccount(3);
  await profilePickerPage.assertLSEngagePage();
});

test('One of the Profile Picker accounts is redirecting correctly 2', async ({ page }) => {
  await profilePickerPage.assertProfilePickerPage();
