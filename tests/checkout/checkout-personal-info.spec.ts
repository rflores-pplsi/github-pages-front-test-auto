...
<<<<<<< HEAD
for (const state of RegionsUtils.usStates.filter(
  (state) => state.abbrv != "MA" && state.priority == true
)) {
  test.only(`Update address and continue to payment page -  ${state.name}`, async ({
    page,
  }) => {
    console.log(
      `Test Case: Update address and continue to payment page -  ${state.name}`
    );
    await checkoutPersonalInfoPage.createOrderRedirectToCheckout(
      "D2C",
      "LegalShield",
      state.name,
      "en-US",
    ...
    await checkoutPersonalInfoPage.navigatePersonalInfoPageFromLogin(
      basicUser.email,
      basicUser.password
    );
    await checkoutPersonalInfoPage.assertPlanNameAndCost(
      "Legal Plan",
      "$29.95"
    );
